...
export default function Home() {
  ...
  const [formattedProofs, setFormattedProofs] =
    useState<ContractProofData | null>(null); // Store formatted proofs
  ...
  const createRequest = async () => {
    ...
    try {
      ...
      onProofGenerated((proofResult: ProofResult) => {
        ...
        console.log(
          `Collected ${collectedProofsRef.current.length} proofs so far`
        );
      });
      ...
      onResult(async (resultData) => {
        ...
        let proofs: ProofResult[] | null =
          ((resultData as Record<string, unknown>).proofs as ProofResult[]) ||
          ((resultData as Record<string, unknown>).proof as ProofResult[]) ||
          ((resultData as Record<string, unknown>)
            .proofResults as ProofResult[]) ||
          null;
        ...
        if (!proofs && collectedProofsRef.current.length > 0) {
          ...
          console.log(
            "Using collected proofs from onProofGenerated:",
            (proofs as ProofResult[]).length,
            "proofs"
          );
        }
        ...
        if (proofs && Array.isArray(proofs) && proofs.length > 0) {
          ...
          try {
            contractProofData = await ZKPassportHelper.formatProofsForContract(
              proofs
            );
            if (contractProofData) {
              ...
              console.error(
                "Failed to format proofs - received null/undefined"
              );
            }
          } catch (formatError) {
          ...
          console.log(
            "Collected proofs count:",
            collectedProofsRef.current.length
          );
        }
        ...
        const serializableProofData = contractProofData
          ? {
              vkeys: {
                vkey_a: contractProofData.vkeys.vkey_a.map((v: bigint) =>
                  v.toString()
                ),
                vkey_b: contractProofData.vkeys.vkey_b.map((v: bigint) =>
                  v.toString()
                ),
                vkey_c: contractProofData.vkeys.vkey_c.map((v: bigint) =>
                  v.toString()
                ),
                vkey_d: contractProofData.vkeys.vkey_d.map((v: bigint) =>
                  v.toString()
                ),
                vkey_e: contractProofData.vkeys.vkey_e.map((v: bigint) =>
                  v.toString()
                ),
              },
              proofs: {
                proof_a: contractProofData.proofs.proof_a.map((p: bigint) =>
                  p.toString()
                ),
                proof_b: contractProofData.proofs.proof_b.map((p: bigint) =>
                  p.toString()
                ),
                proof_c: contractProofData.proofs.proof_c.map((p: bigint) =>
                  p.toString()
                ),
                proof_d: contractProofData.proofs.proof_d.map((p: bigint) =>
                  p.toString()
                ),
                proof_e: contractProofData.proofs.proof_e.map((p: bigint) =>
                  p.toString()
                ),
              },
              vkey_hashes: {
                vkey_hash_a:
                  contractProofData.vkey_hashes.vkey_hash_a.toString(),
                vkey_hash_b:
                  contractProofData.vkey_hashes.vkey_hash_b.toString(),
                vkey_hash_c:
                  contractProofData.vkey_hashes.vkey_hash_c.toString(),
                vkey_hash_d:
                  contractProofData.vkey_hashes.vkey_hash_d.toString(),
                vkey_hash_e:
                  contractProofData.vkey_hashes.vkey_hash_e.toString(),
              },
              public_inputs: {
                input_a: contractProofData.public_inputs.input_a.map(
                  (i: bigint) => i.toString()
                ),
                input_b: contractProofData.public_inputs.input_b.map(
                  (i: bigint) => i.toString()
                ),
                input_c: contractProofData.public_inputs.input_c.map(
                  (i: bigint) => i.toString()
                ),
                input_d: contractProofData.public_inputs.input_d.map(
                  (i: bigint) => i.toString()
                ),
                input_e: contractProofData.public_inputs.input_e.map(
                  (i: bigint) => i.toString()
                ),
              },
            }
        ...
        // Send data to API
        setTxStatus(
          `Processing your verified donation of ${finalDonationAmount} tokens...`
        );
        setTxHash("");
        ...
        try {
          ...
          if (data.success) {
            setTxStatus(
              `Your verified donation of ${finalDonationAmount} tokens has been submitted!`
            );
  ...
  const startPollingDonation = (currentTxHash?: string) => {
    ...
    setIsPolling(true);
    setTxStatus(
      (prevStatus) => `${prevStatus} - Checking donation confirmation...`
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Make verified donations using zero-knowledge identity proofs and
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500">
                  Your donation will be securely processed after identity
                  verification (1-254 tokens)
                </p>
                ...
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-green-600">
                        Your identity has been verified without revealing
                        personal data
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border border-green-100">
                  ...
                  <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    {submittedAmount === receivedDonation ? (
                      <p className="text-xs text-green-600 mt-2 text-center">
                        ✅ Donation amount verified!
                      </p>
                    ) : (
                      <p className="text-xs text-red-600 mt-2 text-center">
                        ⚠️ Amounts don&apos;t match (Expected: {submittedAmount}
                        , Got: {receivedDonation})
                      </p>
                    )}
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-green-100">
                    <p className="text-sm text-green-600 text-center">
                      🌟 Thank you for your verified donation! Your contribution
                      has been securely processed across blockchains.
                    </p>
                  </div>
                </div>
              </div>
