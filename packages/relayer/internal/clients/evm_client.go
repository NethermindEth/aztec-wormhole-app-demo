...
// EVMClientInterface defines the interface for EVM-compatible blockchain client
...
func NewEVMClient(rpcURL, privateKeyHex string, log *logger.Logger) (*EVMClient, error) {
	client := &EVMClient{
		logger: log.WithComponent("EVMClient"),
	}
	...
	client.logger.Info("EVM client initialized", zap.String("address", address.Hex()))
...
func (c *EVMClient) SendVerifyTransaction(ctx context.Context, targetContract string, vaaBytes []byte) (string, error) {
	...
	c.logger.Info("Transaction sent", zap.String("txHash", txHash))
